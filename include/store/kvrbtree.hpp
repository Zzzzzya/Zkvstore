...
namespace zkv{
    ...
    struct ms{
        ...
        friend std::ostream& operator<<(std::ostream& os, const ms& obj) {
            os << "(" << obj.member << ", " << obj.score << ")";
            return os;
        }
    ...
    template<typename Type>
    std::pair<bool, typename Type::score_type> rbtree<Type>::checkmember(Type::member_type mem){
    ...
    {   
        ...
        */
        ...
        while(local != Nil){
            ...
            if(local->data == value){
                //已经存在key为value的结点 插入失败
                std::cout<<"该值已存在！插入失败！"<<std::endl;
                return false;
            }
    ...
    {
        //层序遍历
        std::queue<std::pair<rbnode<Type>*,int>> que;

        int level_index=0;

        if(root == Nil) 
        {
            std::cout<<"黑Nil"<<std::endl;
            return;
        }
        ...
        while(!que.empty()){
            ...
            if(node->color == RED)std::cout<<"红";
            else std::cout<<"黑";

            if(node == Nil)std::cout<<"Nil"<<" ";
